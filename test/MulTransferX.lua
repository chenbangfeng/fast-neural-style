...
--获取去后缀文件名
local function StripPathEx(filename)
...
--			local strmodelname = string.sub(transfer_model_files[i], 17, string.len(transfer_model_files[i])-3)
			local strmodelname = StripPathEx(transfer_model_files[i])
--			local strcontentname = string.sub(content_img_files[j], 16, string.len(content_img_files[j])-4)
			local strcontentname = StripPathEx(content_img_files[j])
			saveIm(params.output_path..'/'..strcontentname.."_"..strmodelname..'.jpg', newimg)
